...
import { LedgerWiseOutstandingReport } from "./components/reports/outstanding/ledger-wise-outstanding/LedgerWiseOutstandingReport";
import { LedgerAgeingReport } from "./components/reports/outstanding/ledger-ageing-report/LedgerAgeingReport";
import { LedgerRegisterReport } from "./components/reports/register/ledger-register/LedgerRegisterReport";
import { SalesRegisterReport } from "./components/reports/register/SalesRegisterReport";
import { PurchaseRegisterReport } from "./components/reports/register/PurchaseRegisterReport";
...
import { StockValuationReport } from "./components/reports/stock-valuation/StockValuationReport";
...
import { SalesOrderSummaryReport } from "./components/reports/sales-order-summary/SalesOrderSummaryReport";
...
function AppContent() {
  ...
  const [tabs, setTabs] = useState<Tab[]>(() => {
    const saved = sessionStorage.getItem('erp_tabs');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return [{ id: "dashboard", type: "dashboard", title: "Dashboard", closable: false }];
  });
  const [activeTabId, setActiveTabId] = useState(() => {
    return sessionStorage.getItem('erp_activeTab') || "dashboard";
  });
  const [splitMode, setSplitMode] = useState<SplitMode>(() => {
    return (sessionStorage.getItem('erp_splitMode') as SplitMode) || "single";
  });
  const [paneActiveTabIds, setPaneActiveTabIds] = useState<string[]>(() => {
    const saved = sessionStorage.getItem('erp_paneActiveTabs');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return ["dashboard"];
  ...
  const [pendingGroupSave, setPendingGroupSave] = useState<{
    record: GroupRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editExtraChargeId, setEditExtraChargeId] = useState<number | null>(
    null,
  );
  const [pendingExtraChargeSave, setPendingExtraChargeSave] = useState<{
    record: ExtraChargeRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editStockPlaceId, setEditStockPlaceId] = useState<number | null>(null);
  const [pendingStockPlaceSave, setPendingStockPlaceSave] = useState<{
    record: StockPlaceRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editUnitId, setEditUnitId] = useState<number | null>(null);
  const [pendingUnitSave, setPendingUnitSave] = useState<{
    record: UnitRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editCurrencyId, setEditCurrencyId] = useState<number | null>(null);
  const [pendingCurrencySave, setPendingCurrencySave] = useState<{
    record: CurrencyRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editSalesPersonId, setEditSalesPersonId] = useState<number | null>(
    null,
  );
  const [pendingSalesPersonSave, setPendingSalesPersonSave] = useState<{
    record: SalesPersonRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editItemCategoryId, setEditItemCategoryId] = useState<number | null>(
    null,
  );
  const [pendingItemCategorySave, setPendingItemCategorySave] = useState<{
    record: ItemCategoryRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editLedgerTargetId, setEditLedgerTargetId] = useState<number | null>(
    null,
  );
  const [pendingLedgerTargetSave, setPendingLedgerTargetSave] = useState<{
    record: any;
    isUpdate: boolean;
  } | null>(null);
  const [editProjectSiteId, setEditProjectSiteId] = useState<number | null>(
    null,
  );
  const [pendingProjectSiteSave, setPendingProjectSiteSave] = useState<{
    record: ProjectSiteRecord;
    isUpdate: boolean;
  } | null>(null);
  const [editTermsId, setEditTermsId] = useState<number | null>(null);
  const [pendingTermsSave, setPendingTermsSave] = useState<{
    record: TermsConditionRecord;
    isUpdate: boolean;
  } | null>(null);

  // Sync state to sessionStorage
  useEffect(() => {
    sessionStorage.setItem('erp_tabs', JSON.stringify(tabs));
  }, [tabs]);

  useEffect(() => {
    sessionStorage.setItem('erp_activeTab', activeTabId);
  }, [activeTabId]);

  useEffect(() => {
    sessionStorage.setItem('erp_splitMode', splitMode);
  }, [splitMode]);

  useEffect(() => {
    sessionStorage.setItem('erp_paneActiveTabs', JSON.stringify(paneActiveTabIds));
  }, [paneActiveTabIds]);
  ...
  const renderTabContent = (tab: Tab) => {
    switch (tab.type) {
      case "dashboard":
        return (
          <Dashboard onModuleClick={(type, title) => addTab(type, title)} />
        );
      case "invoice-list":
        return (
          <InvoiceList
            onCreateInvoice={() => addTab("invoice-create", "New Invoice")}
          />
        );
      case "invoice-create":
      ...
      case "bom-list":
        return (
          <BOMList onCreateNew={() => addTab("bom-create", "Create BOM")} />
        );
      case "bom-create":
      ...
      case "item-list":
        return (
          <ItemList onCreateNew={() => addTab("item-create", "Create Item")} />
        );
      ...
      case "ledger-list":
        return (
          <LedgerList
            onCreateNew={() => addTab("ledger-create", "Create Ledger")}
          />
        );
      ...
      case "user-list":
        return (
          <UserList onCreateNew={() => addTab("user-create", "Create User")} />
        );
      case "user-create":
      ...
      case "receipt-voucher-list":
        return (
          <ReceiptVoucherList
            onCreateNew={() =>
              addTab("receipt-voucher-create", "New Receipt Voucher")
            }
          />
        );
      ...
      case "payment-voucher-list":
        return (
          <PaymentVoucherList
            onCreateNew={() =>
              addTab("payment-voucher-create", "New Payment Voucher")
            }
          />
        );
      ...
      case "purchase-invoice-list":
        return (
          <PurchaseInvoiceList
            onCreateInvoice={() =>
              addTab("purchase-invoice-create", "New Purchase Invoice")
            }
          />
        );
      ...
      case "schedule-report":
        return (
          <ScheduleReport
            onConvertToInvoice={() =>
              addTab("schedule-to-invoice", "Schedule to Invoice")
            }
          />
        );
      ...
      case "sales-person-outstanding-report":
        return <SalesPersonOutstandingReport />;
      case "ledger-wise-outstanding-report":
        return <LedgerWiseOutstandingReport />;
      case "ledger-ageing-report":
        return <LedgerAgeingReport />;
      case "ledger-register":
        return <LedgerRegisterReport />;
      case "sales-register":
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-200 selection:bg-primary/20 selection:text-primary">
      <Header
        ...
        setCurrentPage={(page) => {
          const titles: Record<Page, string> = {
            ...
            "ledger-wise-outstanding-report": "Ledger Wise Outstanding",
      ...
      <TabBar
        ...
        activeTabId={
          splitMode === "single"
            ? activeTabId
            : paneActiveTabIds[focusedPaneIndex]
        }
      ...
      <main className="pb-20 px-4">
        {splitMode === "single" ? (
          ...
          <div
            className={`grid gap-4 h-[calc(100vh-180px)] ${
              splitMode === "split-v"
                ? "grid-cols-2"
                : splitMode === "split-h"
                  ? "grid-rows-2"
              ...
              return (
                <div
                  ...
                  className={`relative border-2 rounded-2xl overflow-hidden transition-all duration-300 bg-white dark:bg-slate-900 shadow-sm ${
                    ...
                    splitMode === "three-left" && index === 0
                      ? "col-span-2 row-span-2"
                      : ""
                  }`}
                >
                  <div
                    className={`absolute top-0 left-0 right-0 h-1 z-10 ${isFocused ? "bg-primary" : "bg-transparent"}`}
                  />
