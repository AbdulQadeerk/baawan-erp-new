...
const mockVouchers = [
  { id: '1', no: 'PV-2024-042', date: '12 Oct 2024', party: 'Reliance Industries Ltd.', mode: 'Bank Transfer', amount: 145000.00, status: 'Bank Transfer' },
  { id: '2', no: 'PV-2024-041', date: '12 Oct 2024', party: 'Office Supplies Corp', mode: 'Petty Cash', amount: 12500.00, status: 'Petty Cash' },
  { id: '3', no: 'PV-2024-040', date: '11 Oct 2024', party: 'Acme International', mode: 'HDFC Bank', amount: 230000.00, status: 'HDFC Bank' },
  { id: '4', no: 'PV-2024-039', date: '10 Oct 2024', party: 'Global Tech Solutions', mode: 'ICICI Bank', amount: 62500.00, status: 'ICICI Bank' },
];
...
export const PaymentVoucherList: React.FC<PaymentVoucherListProps> = ({ onCreateNew }) => {
  return (
    <motion.div 
    ...
    >
      ...
      <nav className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-400">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-900 dark:text-white">Payment Vouchers</span>
      </nav>

      {/* Page Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Payment Vouchers</h1>
          <p className="text-sm text-slate-500 font-bold uppercase tracking-widest opacity-60 mt-1">Manage and track all vendor payments and cash outflows</p>
        </div>
        <button 
          onClick={onCreateNew}
          className="flex items-center justify-center gap-3 px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl shadow-emerald-600/20 transition-all active:scale-95"
        >
          <Plus size={20} /> Create Payment
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-wrap items-end gap-6">
        <div className="flex-1 min-w-[240px] space-y-2">
          ...
          <div className="relative group">
            <Tag className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-600 transition-colors" size={18} />
            <input 
              className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-emerald-600/10 focus:border-emerald-600 outline-none transition-all" 
        ...
        <div className="flex gap-3">
          ...
          <button className="p-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-emerald-600 rounded-2xl transition-all active:scale-95 shadow-sm">
            <RefreshCw size={20} />
          </button>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {mockVouchers.map((v, i) => (
                <tr key={v.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-8 py-5 text-sm font-black text-emerald-600 uppercase tracking-tight">{v.no}</td>
        ...
        <div className="px-8 py-5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <button className="w-10 h-10 rounded-xl bg-emerald-600 text-white font-black text-xs shadow-lg shadow-emerald-600/20">1</button>
      ...
      <footer className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex items-center gap-12">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Total Payments</span>
            <span className="text-2xl font-black text-emerald-600 uppercase tracking-tight">₹ 4,50,000.00</span>
          </div>
        </div>
        <div className="flex items-center gap-6">
          <button className="flex items-center gap-2 text-[10px] font-black text-slate-500 hover:text-emerald-600 uppercase tracking-widest transition-colors">
...
const PaginationButton = ({ icon, disabled = false }: { icon: React.ReactNode, disabled?: boolean }) => (
  <button 
    disabled={disabled}
    className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-400 hover:text-emerald-600 transition-all disabled:opacity-30 disabled:cursor-not-allowed active:scale-90 shadow-sm"
  >
    {icon}
  </button>
);
