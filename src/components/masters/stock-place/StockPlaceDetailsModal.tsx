...
import { stockPlaceApi, type StockPlaceRecord } from '../../../services/stock-place.service';

interface StockPlaceDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  recordId: number | null;
}

export const StockPlaceDetailsModal: React.FC<StockPlaceDetailsModalProps> = ({
  isOpen,
  onClose,
  recordId,
}) => {
  const [record, setRecord] = useState<StockPlaceRecord | null>(null);
  ...
  async function loadData(id: number) {
    ...
    try {
      ...
      if (!err?._processedByInterceptor) {
        setError(err?.message || 'Failed to load stock place details.');
      }
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="px-8 py-6">
              ...
              {!isLoading && !error && record && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <DetailItem label="Code" value={record.code || 'N/A'} />
                  
                  <DetailItem label="Address 1" value={record.address_1 || 'N/A'} />
                  <DetailItem label="Address 2" value={record.address_2 || 'N/A'} />
                  
                  <DetailItem label="State" value={record.state || 'N/A'} />
                  <DetailItem label="City" value={record.city || 'N/A'} />
                  
                  <DetailItem label="Area" value={record.area || 'N/A'} />
                  <DetailItem label="Pin Code" value={record.pin || 'N/A'} />
                  
                  <div className="sm:col-span-2">
                    <DetailItem label="Phone" value={record.phone || 'N/A'} />
                  </div>
                </div>
              )}
            </div>
