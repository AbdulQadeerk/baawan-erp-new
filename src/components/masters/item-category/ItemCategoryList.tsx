...
import { 
  ...
  type ItemCategorySearchPayload 
} from '../../../services/item-category.service';
import { ItemCategoryDetailsModal } from './ItemCategoryDetailsModal';
import { toast } from '../../../lib/toast';

interface ItemCategoryListProps {
  onCreateNew?: () => void;
  onEdit?: (id: number) => void;
  pendingSave?: { record: ItemCategoryRecord; isUpdate: boolean } | null;
...
export const ItemCategoryList: React.FC<ItemCategoryListProps> = ({ 
  ...
  const [filteredCategories, setFilteredCategories] = useState<ItemCategoryRecord[]>([]);
  ...
  // Delete
  const [deleteTarget, setDeleteTarget] = useState<ItemCategoryRecord | null>(null);
  ...
  useEffect(() => {
    ...
    if (isUpdate) {
      setCategories(prev => prev.map(s => s.priceCategoryID === record.priceCategoryID ? { ...s, ...record } : s));
    } else {
      setCategories(prev => [record, ...prev]);
    }
    onPendingSaveConsumed?.();
  }, [pendingSave]);
  ...
  useEffect(() => {
    ...
    if (searchText.trim()) {
      ...
      result = result.filter(s => 
        s.priceCategoryName?.toLowerCase().includes(term)
      );
    }
  ...
  const fetchData = useCallback(async (textOverride?: string | null) => {
    ...
    try {
      ...
      const payload: ItemCategorySearchPayload = {
        isSync: false,
        name: searchVal
      };
      
      const data = await itemCategoryApi.search(payload);
  ...
  const handleDelete = async (category: ItemCategoryRecord) => {
    try {
      await itemCategoryApi.delete(category.priceCategoryID);
      toast.success('Record deleted successfully.', 'Success');
      setCategories(prev => prev.filter(s => s.priceCategoryID !== category.priceCategoryID));
  ...
  const handleExport = async () => {
    ...
    try {
      const payload: ItemCategorySearchPayload = {
        isSync: false,
        name: searchText.trim() || null
      };
      const blob = await itemCategoryApi.exportToExcel(payload);
      if (blob.size) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.setAttribute('download', 'ItemCategoryExport.xlsx');
        a.click();
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const data = await itemCategoryApi.print(payload);
      if (data?.list?.length) {
        const printWindow = window.open('', '_blank');
        if (printWindow) {
          printWindow.document.write(`
            <html><head><title>Item Rate Category Listings</title>
            ...
            <h2>Item Rate Category Listings</h2>
            <table><thead><tr><th>Name</th></tr></thead><tbody>
            ${data.list.map((r: any) => `<tr><td>${r.priceCategoryName || ''}</td></tr>`).join('')}
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 flex flex-col gap-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-4 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-4">
          <div className="bg-pink-500 p-2.5 rounded-2xl shadow-lg shadow-pink-500/20">
        ...
        <div className="flex flex-wrap items-center gap-2 lg:gap-3">
          ...
          <div className="relative w-full sm:w-64 group">
            <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-pink-600 transition-colors">
              <Search size={16} />
            </span>
            <input
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold focus:ring-2 focus:ring-pink-600/20 focus:border-pink-600 outline-none transition-all shadow-sm"
          ...
          <button
            ...
            disabled={isLoading}
            className="p-2.5 bg-pink-600 text-white rounded-xl hover:bg-pink-700 transition-all shadow-sm active:scale-95 disabled:opacity-50"
            title="Search"
          >
          ...
          {onCreateNew && (
            <button
              onClick={onCreateNew}
              className="p-2.5 bg-pink-600 text-white rounded-xl hover:bg-pink-700 transition-all shadow-sm active:scale-95 shadow-pink-600/20"
              title="Create New"
            >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              {!isLoading && paginatedCategories.map(category => (
                <tr key={category.priceCategoryID} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    <div className="flex items-center justify-center gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setDetailRecordId(category.priceCategoryID); setDetailModalOpen(true); }} className="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 p-2 rounded-xl transition-all active:scale-90" title="View Details"><Eye size={18} /></button>
                      <button onClick={() => onEdit?.(category.priceCategoryID)} className="text-slate-500 hover:text-pink-600 hover:bg-pink-50 dark:hover:bg-pink-900/20 p-2 rounded-xl transition-all active:scale-90" title="Edit"><Edit3 size={18} /></button>
                      <button onClick={() => setDeleteTarget(category)} className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 p-2 rounded-xl transition-all active:scale-90" title="Delete"><Trash2 size={18} /></button>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight text-sm">
                    {category.priceCategoryName}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="px-8 py-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-10">
            <SummaryStat label="Total Rows" value={String(categories.length)} />
            <SummaryStat label="Filtered" value={String(filteredCategories.length)} color="text-pink-600" />
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-4">
      ...
      <ItemCategoryDetailsModal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} recordId={detailRecordId} />
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-700/50 w-full max-w-md mx-4 p-8">
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Are you sure you want to delete <strong className="text-slate-700 dark:text-slate-200">{deleteTarget.priceCategoryName}</strong>? This action cannot be undone.</p>
