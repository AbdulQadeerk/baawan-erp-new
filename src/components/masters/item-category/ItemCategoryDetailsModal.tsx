...
interface ItemCategoryDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  recordId: number | null;
}

export const ItemCategoryDetailsModal: React.FC<ItemCategoryDetailsModalProps> = ({
  isOpen,
  onClose,
  recordId,
}) => {
  const [record, setRecord] = useState<ItemCategoryRecord | null>(null);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <motion.div
            ...
            className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-700/50 w-full max-w-sm mx-4 overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="px-8 py-6">
              ...
              {!isLoading && !error && record && (
                <div className="grid grid-cols-1 gap-6">
                  <DetailItem label="Name" value={record.priceCategoryName || 'N/A'} />
                </div>
              )}
            </div>
