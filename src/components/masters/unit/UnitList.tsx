...
interface UnitListProps {
  ...
  pendingSave?: { record: UnitRecord; isUpdate: boolean } | null;
...
export const UnitList: React.FC<UnitListProps> = ({ 
  ...
  const [filteredUnits, setFilteredUnits] = useState<UnitRecord[]>([]);
  ...
  useEffect(() => {
    ...
    if (isUpdate) {
      setUnits(prev => prev.map(u => u.id === record.id ? { ...u, ...record } : u));
    } else {
      setUnits(prev => [record, ...prev]);
    }
    onPendingSaveConsumed?.();
  }, [pendingSave]);
  ...
  useEffect(() => {
    ...
    if (searchCategory) {
      result = result.filter(u => u.category === searchCategory);
    }
    setFilteredUnits(result);
    setCurrentPage(1);
  }, [units, searchText, searchCategory]);

  const fetchData = useCallback(async (textOverride?: string | null, catOverride?: number | null) => {
    setIsLoading(true);
    try {
      const payload: UnitSearchPayload = {
        isSync: false,
        name: textOverride !== undefined ? textOverride : (searchText.trim() || null),
        category: catOverride !== undefined ? catOverride : (searchCategory || null)
      };
      const data = await unitApi.search(payload);
      setUnits(data.list || []);
    } catch (err: any) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const blob = await unitApi.exportToExcel(payload);
      if (blob.size) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.setAttribute('download', 'UnitMasterExport.xlsx');
        a.click();
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const data = await unitApi.print(payload);
      if (data?.list?.length) {
        const printWindow = window.open('', '_blank');
        if (printWindow) {
          printWindow.document.write(`
            <html><head><title>Unit Master List</title>
            ...
            <h2>Unit Master List</h2>
            <table><thead><tr><th>Name</th><th>Category</th><th>Short Name</th></tr></thead><tbody>
            ${data.list.map((r: any) => `<tr><td>${r.name || ''}</td><td>${getUnitCategoryText(r.category)}</td><td>${r.shortName || ''}</td></tr>`).join('')}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-center gap-2 lg:gap-3">
          ...
          <div className="relative w-full sm:w-40 group">
            <select
              ...
              onChange={(e) => setSearchCategory(e.target.value ? Number(e.target.value) : null)}
              className="w-full pl-4 pr-8 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 outline-none transition-all shadow-sm appearance-none cursor-pointer text-slate-700 dark:text-slate-300"
            >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-auto custom-scrollbar" style={{ maxHeight: '60vh' }}>
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              {!isLoading && paginatedUnits.length === 0 && (
                <tr>
                  <td colSpan={4} className="text-center py-16">
                    <div className="flex flex-col items-center gap-3 text-slate-400">
                      <Box size={40} className="opacity-30" />
                      <span className="text-sm font-bold uppercase tracking-widest">No units found</span>
                    </div>
                  </td>
                </tr>
              )}

              {!isLoading && paginatedUnits.map(unit => (
                <tr key={unit.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    <div className="flex items-center justify-center gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setDetailRecordId(unit.id); setDetailModalOpen(true); }} className="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 p-2 rounded-xl transition-all active:scale-90" title="View Details"><Eye size={18} /></button>
                      <button onClick={() => onEdit?.(unit.id)} className="text-slate-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 p-2 rounded-xl transition-all active:scale-90" title="Edit"><Edit3 size={18} /></button>
                      <button onClick={() => setDeleteTarget(unit)} className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 p-2 rounded-xl transition-all active:scale-90" title="Delete"><Trash2 size={18} /></button>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span className="px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border bg-slate-50 text-slate-700 border-slate-100 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700">
                      {getUnitCategoryText(unit.category)}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-mono font-black text-slate-600 dark:text-slate-400 text-sm tracking-wider">
                    {unit.shortName}
                  </td>
                </tr>
              ))}
        ...
        <div className="px-8 py-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-10">
            <SummaryStat label="Total Rows" value={String(units.length)} />
            <SummaryStat label="Filtered" value={String(filteredUnits.length)} color="text-blue-600" />
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-4">
      ...
      <UnitDetailsModal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} recordId={detailRecordId} />
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-700/50 w-full max-w-md mx-4 p-8">
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Are you sure you want to delete <strong className="text-slate-700 dark:text-slate-200">{deleteTarget.name}</strong>? This action cannot be undone.</p>
