...
import { 
  termsConditionApi, 
  type TermsConditionRecord, 
  type TermsConditionSearchPayload 
} from '../../../services/terms-condition.service';
import { TermsDetailsModal } from './TermsDetailsModal';
import { toast } from '../../../lib/toast';

interface TermsListProps {
  onCreateNew?: () => void;
  onEdit?: (id: number) => void;
  pendingSave?: { record: TermsConditionRecord; isUpdate: boolean } | null;
...
export const TermsList: React.FC<TermsListProps> = ({ 
  ...
  const [terms, setTerms] = useState<TermsConditionRecord[]>([]);
  const [filteredTerms, setFilteredTerms] = useState<TermsConditionRecord[]>([]);
  ...
  // Delete
  const [deleteTarget, setDeleteTarget] = useState<TermsConditionRecord | null>(null);
  ...
  useEffect(() => {
    ...
    if (isUpdate) {
      setTerms(prev => prev.map(s => s.id === record.id ? { ...s, ...record } : s));
    } else {
      setTerms(prev => [record, ...prev]);
    }
    onPendingSaveConsumed?.();
  }, [pendingSave]);
  ...
  useEffect(() => {
    ...
    if (searchText.trim()) {
      ...
      result = result.filter(s => 
        s.tncText?.toLowerCase().includes(term)
      );
    }
  ...
  const fetchData = useCallback(async (textOverride?: string | null) => {
    ...
    try {
      ...
      const payload: TermsConditionSearchPayload = {
        isSync: false,
        text: searchVal
      };
      
      const data = await termsConditionApi.search(payload);
      setTerms(data.list || []);
    } catch (err: any) {
  ...
  const handleDelete = async (termObj: TermsConditionRecord) => {
    try {
      await termsConditionApi.delete(termObj.id);
      toast.success('Record deleted successfully.', 'Success');
      setTerms(prev => prev.filter(s => s.id !== termObj.id));
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const blob = await termsConditionApi.exportToExcel(payload);
      if (blob.size) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.setAttribute('download', 'RecordExport.xlsx');
        a.click();
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const data = await termsConditionApi.print(payload);
      if (data?.list?.length) {
        const printWindow = window.open('', '_blank');
        if (printWindow) {
          printWindow.document.write(`
            <html><head><title>Terms Condition Listings</title>
            ...
            <h2>Terms Condition Listings</h2>
            <table><thead><tr><th>Terms Conditions Text</th></tr></thead><tbody>
            ${data.list.map((r: any) => `<tr><td>${r.tncText || ''}</td></tr>`).join('')}
  ...
  const paginatedTerms = filteredTerms.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 flex flex-col gap-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-4 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex flex-wrap items-center gap-2 lg:gap-3">
          ...
          <div className="relative w-full sm:w-64 group">
            ...
            <input
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 outline-none transition-all shadow-sm"
          ...
          <button
            ...
            disabled={isLoading}
            className="p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-sm active:scale-95 disabled:opacity-50"
            title="Search"
          >
          ...
          {onCreateNew && (
            <button
              onClick={onCreateNew}
              className="p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-sm active:scale-95 shadow-indigo-600/20"
              title="Create New"
            >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-auto custom-scrollbar" style={{ maxHeight: '60vh' }}>
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              {!isLoading && paginatedTerms.length === 0 && (
                <tr>
                  <td colSpan={2} className="text-center py-16">
                    <div className="flex flex-col items-center gap-3 text-slate-400">
                      <FileBadge size={40} className="opacity-30" />
              ...
              {!isLoading && paginatedTerms.map(termObj => (
                <tr key={termObj.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    <div className="flex items-center justify-center gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setDetailRecordId(termObj.id); setDetailModalOpen(true); }} className="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 p-2 rounded-xl transition-all active:scale-90" title="View Details"><Eye size={18} /></button>
                      <button onClick={() => onEdit?.(termObj.id)} className="text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 p-2 rounded-xl transition-all active:scale-90" title="Edit"><Edit3 size={18} /></button>
                      <button onClick={() => setDeleteTarget(termObj)} className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 p-2 rounded-xl transition-all active:scale-90" title="Delete"><Trash2 size={18} /></button>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-black text-slate-800 dark:text-slate-200 text-sm whitespace-pre-wrap max-w-2xl">
                    <div className="line-clamp-2" title={termObj.tncText}>
                      {termObj.tncText}
                    </div>
                  </td>
                </tr>
        ...
        <div className="px-8 py-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-10">
            <SummaryStat label="Total Rows" value={String(terms.length)} />
            <SummaryStat label="Filtered" value={String(filteredTerms.length)} color="text-indigo-600" />
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-4">
      ...
      <TermsDetailsModal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} recordId={detailRecordId} />
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-700/50 w-full max-w-md mx-4 p-8">
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Are you sure you want to delete this clause? This action cannot be undone.</p>
