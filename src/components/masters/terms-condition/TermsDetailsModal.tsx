...
interface TermsDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  recordId: number | null;
}

export const TermsDetailsModal: React.FC<TermsDetailsModalProps> = ({
  isOpen,
  onClose,
  recordId,
}) => {
  const [record, setRecord] = useState<TermsConditionRecord | null>(null);
            ...
            <div className="px-8 py-6">
              ...
              {!isLoading && !error && record && (
                <div className="grid grid-cols-1 gap-6">
                  <DetailItem label="Text" value={record.tncText || 'N/A'} />
                </div>
              )}
            </div>
...
const DetailItem = ({ label, value }: { label: string; value: string }) => (
  <div className="space-y-1.5 p-3 rounded-2xl bg-slate-50/50 dark:bg-slate-800/20 border border-slate-100 dark:border-slate-700/50">
    ...
    <span className="block text-sm font-bold text-slate-800 dark:text-slate-200 whitespace-pre-wrap">
      {value}
    </span>
  </div>
);
