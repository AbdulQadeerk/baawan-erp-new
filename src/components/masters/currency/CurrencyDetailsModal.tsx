...
export const CurrencyDetailsModal: React.FC<CurrencyDetailsModalProps> = ({
  isOpen,
  onClose,
  recordId,
}) => {
  ...
  async function loadData(id: number) {
    ...
    try {
      ...
      if (!err?._processedByInterceptor) {
        setError(err?.message || 'Failed to load currency details.');
      }
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="px-8 py-6">
              ...
              {!isLoading && !error && record && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <DetailItem label="Code" value={record.code || 'N/A'} isCode />
                  <DetailItem label="Symbol" value={record.symbol || 'N/A'} isSymbol />
                </div>
              )}
            </div>
...
const DetailItem = ({ label, value, isCode, isSymbol }: { label: string; value: string, isCode?: boolean, isSymbol?: boolean }) => (
  <div className="space-y-1.5 p-3 rounded-2xl bg-slate-50/50 dark:bg-slate-800/20 border border-slate-100 dark:border-slate-700/50">
    ...
    <span className={`block text-sm font-bold ${isSymbol ? 'text-emerald-600 dark:text-emerald-400 text-lg' : isCode ? 'font-mono text-slate-600 dark:text-slate-400 uppercase tracking-wider' : 'text-slate-800 dark:text-slate-200'}`}>
      {value}
    </span>
  </div>
);
