...
interface CurrencyListProps {
  ...
  pendingSave?: { record: CurrencyRecord; isUpdate: boolean } | null;
...
export const CurrencyList: React.FC<CurrencyListProps> = ({ 
  ...
  const [filteredCurrencies, setFilteredCurrencies] = useState<CurrencyRecord[]>([]);
  ...
  useEffect(() => {
    ...
    if (isUpdate) {
      setCurrencies(prev => prev.map(c => c.id === record.id ? { ...c, ...record } : c));
    } else {
      setCurrencies(prev => [record, ...prev]);
    }
    onPendingSaveConsumed?.();
  }, [pendingSave]);
  ...
  const fetchData = useCallback(async (textOverride?: string | null) => {
    setIsLoading(true);
    try {
      const payload: CurrencySearchPayload = {
        isSync: false,
        name: textOverride !== undefined ? textOverride : (searchText.trim() || null)
      };
      const data = await currencyApi.search(payload);
  ...
  const handleDelete = async (currency: CurrencyRecord) => {
    try {
      await currencyApi.delete(currency.id);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const blob = await currencyApi.exportToExcel(payload);
      if (blob.size) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.setAttribute('download', 'CurrencyExport.xlsx');
        a.click();
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const data = await currencyApi.print(payload);
      if (data?.list?.length) {
        const printWindow = window.open('', '_blank');
        if (printWindow) {
          printWindow.document.write(`
            <html><head><title>Currency Listings</title>
            ...
            <h2>Currency Listings</h2>
            <table><thead><tr><th>Name</th><th>Code</th><th>Symbol</th></tr></thead><tbody>
            ${data.list.map((r: any) => `<tr><td>${r.name || ''}</td><td>${r.code || ''}</td><td>${r.symbol || ''}</td></tr>`).join('')}
  ...
  const paginatedCurrencies = filteredCurrencies.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 flex flex-col gap-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50 dark:bg-slate-800/90 border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 w-1/3">
                  Name
                </th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 w-1/4">
                  Code
                </th>
                ...
                <tr>
                  <td colSpan={4} className="text-center py-16">
                    <div className="flex flex-col items-center gap-3 text-slate-400">
                      ...
                      <span className="text-sm font-bold uppercase tracking-widest">No currencies found</span>
                    </div>
                  </td>
                </tr>
              )}

              {!isLoading && paginatedCurrencies.map(currency => (
                <tr key={currency.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    <div className="flex items-center justify-center gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setDetailRecordId(currency.id); setDetailModalOpen(true); }} className="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 p-2 rounded-xl transition-all active:scale-90" title="View Details"><Eye size={18} /></button>
                      <button onClick={() => onEdit?.(currency.id)} className="text-slate-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 p-2 rounded-xl transition-all active:scale-90" title="Edit"><Edit3 size={18} /></button>
                      <button onClick={() => setDeleteTarget(currency)} className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 p-2 rounded-xl transition-all active:scale-90" title="Delete"><Trash2 size={18} /></button>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight text-sm">
                    {currency.name}
                  </td>
                  <td className="px-6 py-4 font-mono font-bold text-slate-600 dark:text-slate-400 text-sm tracking-wider uppercase">
                    {currency.code}
                  </td>
                  <td className="px-6 py-4 font-black text-emerald-600 dark:text-emerald-400 text-lg">
                    {currency.symbol}
                  </td>
                </tr>
              ))}
        ...
        <div className="px-8 py-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-10">
            <SummaryStat label="Total Rows" value={String(currencies.length)} />
            <SummaryStat label="Filtered" value={String(filteredCurrencies.length)} color="text-blue-600" />
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-4">
      ...
      <CurrencyDetailsModal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} recordId={detailRecordId} />
