  ...
  return (
    <div className="space-y-1 relative" ref={ref}>
      ...
      {open && filtered.length > 0 && !disabled && (
        <div
          className={`absolute z-50 top-full mt-1 ${["item", "ledger"].includes(templateType) ? "w-[800px] max-w-[90vw]" : "w-full"} bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl max-h-64 overflow-y-auto`}
        >
          {filtered.map((opt, i) => (
            <button
              key={i}
...
export const ItemRegisterReport: React.FC = () => {
  ...
  const getFiltersPayload = useCallback(() => {
    return {
      fromDate: filters.fromDate
        ? formatDateForApi(filters.fromDate, "00:00:00")
        : null,
      toDate: filters.toDate
        ? formatDateForApi(filters.toDate, "23:59:59")
        : null,
      isOpeningStock: filters.isOpeningStock,
      stockDetail: filters.stockDetail,
      spIds:
        !filters.isOpeningStock && filters.spIds
          ? [parseInt(filters.spIds)]
          : null,
      itemId: filters.item ? filters.item.iid : null,
      mfrItemName: filters.mfrItemName
        ? filters.mfrItemName.name || filters.mfrItemName
        : null,
    };
  }, [filters]);
  ...
  const calculateMonthlyTotals = (sourceData: any[]) => {
    ...
    sortedData.forEach((transaction) => {
      ...
      const inwardsValue =
        (transaction.Received || 0) * (transaction.NetRate || 0);
      const outwardsQuantity = transaction.Issued || 0;
      const outwardsValue =
        (transaction.Issued || 0) * (transaction.NetRate || 0);
    ...
    setMonthlyTotals(
      Array.from(map.values()).filter((m) => m.month !== "January 1970"),
    );
  };
        ...
        <div className="flex flex-wrap xl:flex-nowrap items-start gap-4">
          ...
          <div className="w-full sm:w-40 shrink-0 space-y-1">
            ...
            <select
              value={filters.spIds}
              onChange={(e) =>
                setFilters({ ...filters, spIds: e.target.value })
              }
              disabled={filters.isOpeningStock}
          ...
          {/* Checkboxes (Stacked) */}
          <div className="flex flex-col gap-1 shrink-0 pt-1 xl:ml-2">
            <CheckboxInput
              id="isOpeningStock"
              label="All Stock Places"
              checked={filters.isOpeningStock}
              onChange={(v) =>
                setFilters({
                  ...filters,
                  isOpeningStock: v,
                  spIds: v ? "" : filters.spIds,
                })
              }
            />
            <CheckboxInput
              id="stockDetail"
              label="Stock Balance In Details"
              checked={filters.stockDetail}
              onChange={(v) => setFilters({ ...filters, stockDetail: v })}
            />
            <CheckboxInput
              id="monthWise"
              label="Month Wise"
              checked={filters.monthWise}
              onChange={(v) => setFilters({ ...filters, monthWise: v })}
            />
          </div>

          {/* Buttons */}
          <div className="flex items-center space-x-2 shrink-0 pt-4 xl:pt-1 xl:ml-auto pb-0.5">
            <button
            ...
            <button
              disabled={
                loading ||
                (!filters.monthWise ? !data.length : !monthlyTotals.length)
              }
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  <th
                    rowSpan={2}
                    className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500 border-r border-slate-200 dark:border-slate-700"
                  >
                    Month
                  </th>
                  <th
                    colSpan={2}
                    className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-center text-slate-500 border-r border-slate-200 dark:border-slate-700 bg-blue-50/50 dark:bg-blue-900/10"
                  >
                    Inwards
                  </th>
                  <th
                    colSpan={2}
                    className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-center text-slate-500 border-r border-slate-200 dark:border-slate-700 bg-rose-50/50 dark:bg-rose-900/10"
                  >
                    Outwards
                  </th>
                  <th
                    colSpan={2}
                    className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-center text-slate-500 border-r border-slate-200 dark:border-slate-700 bg-emerald-50/50 dark:bg-emerald-900/10"
                  >
                    Closing
                  </th>
                </tr>
                <tr>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-right text-slate-500 bg-blue-50/50 dark:bg-blue-900/10">
                    Quantity
                  </th>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-right text-slate-500 border-r border-slate-200 dark:border-slate-700 bg-blue-50/50 dark:bg-blue-900/10">
                    Value
                  </th>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-right text-slate-500 bg-rose-50/50 dark:bg-rose-900/10">
                    Quantity
                  </th>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-right text-slate-500 border-r border-slate-200 dark:border-slate-700 bg-rose-50/50 dark:bg-rose-900/10">
                    Value
                  </th>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-right text-slate-500 bg-emerald-50/50 dark:bg-emerald-900/10">
                    Quantity
                  </th>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-right text-slate-500 border-r border-slate-200 dark:border-slate-700 bg-emerald-50/50 dark:bg-emerald-900/10">
                    Value
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {loading ? (
                  ...
                  monthlyTotals.map((mt, idx) => (
                    ...
                    >
                      <td className="px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300 border-r border-slate-100 dark:border-slate-800">
                        {mt.month}
                      </td>
                      <td className="px-4 py-3 text-sm text-right text-slate-600 dark:text-slate-400">
                        {formatNumber(mt.inwardsQuantity)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right text-slate-600 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800">
                        {formatNumber(mt.inwardsValue)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right text-slate-600 dark:text-slate-400">
                        {formatNumber(mt.outwardsQuantity)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right text-slate-600 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800">
                        {formatNumber(mt.outwardsValue)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right font-medium text-slate-700 dark:text-slate-300">
                        {formatNumber(mt.closingQuantity)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right font-medium text-slate-700 dark:text-slate-300 border-r border-slate-100 dark:border-slate-800">
                        {formatNumber(mt.closingValue)}
                      </td>
                    </tr>
                  ))
                )}
                ...
                <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                  ...
                  {columns
                    ...
                    .map((col) => (
                      <th
                        key={col}
                        className={`px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500 whitespace-nowrap ${["Balance", "Received", "Issued"].includes(col) ? "text-right" : ""}`}
                      >
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {loading ? (
                  ...
                  <tr>
                    ...
                    >
                      {submitted
                        ? "No register values found for selected criteria"
                        : "Select an item and click Search"}
                    </td>
                  </tr>
                ) : (
                  data.map((row, idx) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        {row.TypeId && row.Invcode && (
                          <button
                            onClick={() => openInvoiceModal(row, idx)}
                      ...
                      {columns
                        ...
                        .map((col) => (
                          <td
                            key={col}
                            className={`px-4 py-3 text-sm ${["Balance", "Received", "Issued"].includes(col) ? "text-right font-medium text-slate-700 dark:text-slate-300" : "text-slate-600 dark:text-slate-400"} whitespace-nowrap`}
                          >
                            {col === "BillDate"
                              ? formatDateStr(row[col])
                              : ["Balance", "Received", "Issued"].includes(col)
        ...
        <div className="px-5 py-3 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-500 font-medium">
                {filters.monthWise ? "Total Months:" : "Total Rows:"}
              </span>
