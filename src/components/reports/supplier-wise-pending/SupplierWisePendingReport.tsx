...
export const SupplierWisePendingReport: React.FC = () => {
  ...
  const submitReport = async () => {
    ...
    setError(null);
    try {
      const result = await reportApi.supplierWisePendingReport(
        getFormattedFilters(),
      );
      setData(result || []);
  ...
  const handleExport = async () => {
    ...
    try {
      const exportData = await reportApi.supplierWisePendingReportExport(
        getFormattedFilters(),
      );
      if (exportData) {
  ...
  // Subtotals
  const totalPurchaseQty = data.reduce(
    (s, i) => s + (parseFloat(i.PurchaseQty) || 0),
    0,
  );
  const totalInwardPending = data.reduce(
    (s, i) => s + (parseFloat(i.InwardPending) || 0),
    0,
  );
  const totalOutwardPending = data.reduce(
    (s, i) => s + (parseFloat(i.OutwardPending) || 0),
    0,
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              Supplier wise pending inward and outward tracking.
            </p>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                data.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-xs font-bold text-blue-600 dark:text-blue-400 text-right">
                      {formatNumber(item.PurchaseQty)}
                    </td>
                    <td className="px-4 py-2 text-xs font-bold text-orange-600 dark:text-orange-400 text-right">
                      {formatNumber(item.InwardPending)}
                    </td>
                    <td className="px-4 py-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 text-right">
                      {formatNumber(item.OutwardPending)}
                    </td>
                  </tr>
                ))
              )}
