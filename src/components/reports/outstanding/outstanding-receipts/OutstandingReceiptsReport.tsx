...
import { Search, RotateCcw, FileSpreadsheet, Loader2, Receipt, Eye, Printer } from 'lucide-react';
...
export const OutstandingReceiptsReport: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (blob?.size) {
        ...
        a.download = 'outstanding-receiptReport.xlsx';
        a.click();
  ...
  const isVoucherObject = (value: any): boolean => Array.isArray(value);
    ...
    <div className="font-sans text-slate-700 dark:text-slate-200">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="bg-emerald-100 dark:bg-emerald-900/30 p-2.5 rounded-xl text-emerald-600 dark:text-emerald-400"><Receipt size={22} /></div>
          <div>
            <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">Outstanding Receipts</h1>
            <p className="text-xs text-slate-500 font-medium">All pending receipt outstanding</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div>
            ...
            <input type="date" value={toDate} onChange={e => setToDate(e.target.value)}
              className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all w-44" />
          </div>
        ...
        <div className="space-y-4">
          {uniqueLedLst.map((item, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-slate-800 dark:to-slate-800/80 p-3 border-b border-slate-200 dark:border-slate-700">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {item.data.map((row: any, ri: number) => (
                      <tr key={ri} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                        <td className="px-4 py-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400">{row.billNo}</td>
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex gap-6">
                    ...
                    <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Pending: {H.formatNumber(H.getLedgerPending(item.data, precision), precision)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="sticky bottom-0 z-10 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-[0_-4px_12px_-2px_rgba(0,0,0,0.05)] p-4">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-8">
                ...
                <div className="text-right">
                  <span className="text-[10px] text-emerald-500 font-bold uppercase tracking-wider block mb-1">Pending Amount</span>
                  <span className="text-lg font-black text-emerald-600 dark:text-emerald-400">{H.formatNumber(pendingAmount, precision)}</span>
                </div>
              </div>
            </div>
          </div>
