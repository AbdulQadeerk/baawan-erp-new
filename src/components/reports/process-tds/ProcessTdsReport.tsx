...
export const ProcessTdsReport: React.FC = () => {
  ...
  // Load Sundry Creditors / target Ledgers (Group 16)
  useEffect(() => {
    const targetLedgers = H.getFilteredLedgers([16]);
    setLedgers(targetLedgers);
  }, []);
  ...
  const submitSearch = useCallback(async () => {
    ...
    try {
      ...
      const data = await reportApi.reportProcessTDS(filters);
  ...
  const handleExport = async () => {
    ...
    try {
      const filters = getFilters();
      const blob = await reportApi.reportProcessTDSExport(filters);
      if (blob?.size) {
        const a = document.createElement("a");
        a.href = URL.createObjectURL(blob);
        a.download = "process-tdsReport.xlsx";
        a.click();
  ...
  const handlePrint = async () => {
    ...
    try {
      const printData = lst.length
        ? lst
        : await reportApi.reportProcessTDS(getFilters());
  ...
  const handleCreateCN = (e: React.FormEvent) => {
    ...
    storage.setItem(STORAGE_KEYS.PROCESS_TDS, processTDS);

    toast.success(
      "TDS processed successfully! Data saved to localStorage. Redirecting to credit note...",
      "Success"
    );

    setTimeout(() => {
      window.location.href = "/voucher/credit-note/new";
    }, 1500);
  };
    ...
    <div className="font-sans text-slate-700 dark:text-slate-200 pt-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">Process TDS</h1>
            <p className="text-xs text-slate-500 font-medium">
              Calculate customer TDS transaction values and create credit vouchers.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
        <div className="flex flex-wrap items-end gap-3 w-full">
          ...
          {lst.length > 0 && (
            <form onSubmit={handleCreateCN} className="flex items-end gap-2 bg-slate-50 dark:bg-slate-800/25 p-3 rounded-lg border border-slate-200 dark:border-slate-800 ml-auto">
