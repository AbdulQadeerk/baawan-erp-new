  ...
  return (
    <div className="space-y-1 relative" ref={ref}>
      ...
      <input
        ...
        className={`w-full px-3 py-[7px] bg-slate-50 dark:bg-slate-800 border rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all disabled:opacity-50 ${hasError ? "border-red-500 focus:border-red-500" : "border-slate-200 dark:border-slate-700 focus:border-blue-500"}`}
      />
...
export const PendingReport: React.FC = () => {
  ...
  useEffect(() => {
    ...
    commonApi
      .dropdown({ table: 5 })
      .then(setSalesPersonList)
      .catch(() => {});

    // Load additional lists
    commonApi
      .itemCategoryList({ table: 0, column: "Brand" })
      .then(setBrandList)
      .catch(() => {});
    commonApi
      .itemCategoryList({ table: 0, column: "Category" })
      .then(setCategoryList)
      .catch(() => {});
    commonApi
      .itemCategoryList({ table: 0, column: "Sizes" })
      .then(setSizeList)
      .catch(() => {});
    commonApi
      .itemCategoryList({ table: 0, column: "Type" })
      .then(setTypeList)
      .catch(() => {});
    commonApi
      .itemCategoryList({ table: 0, column: "ItemGroup" })
      .then(setGroupList)
      .catch(() => {});
    commonApi
      .itemCategoryList({ table: 0, column: "Name" })
      .then(setItemNameList)
      .catch(() => {});
    commonApi
      .itemCategoryList({ table: 0, column: "ItemCode" })
      .then(setItemCodeList)
      .catch(() => {});
  }, []);
  ...
  const invoiceListForModal = data
    .filter((r) => r.InvoiceCode && r.TypeId)
    .map((r) => ({ ...r, Invcode: r.InvoiceCode }));
  ...
  return (
    <div className="font-sans text-slate-700 dark:text-slate-200 pt-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-5">
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 mb-2">
          ...
          <CheckboxInput
            ...
            onChange={(v) => setFilters({ ...filters, billDetailsReq: v })}
          />
          <CheckboxInput
            id="billWise"
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, invType: e.target.value })
              }
              disabled={filters.billWise}
              className={`w-full px-3 py-[7px] bg-slate-50 dark:bg-slate-800 border rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${submitted && !filters.billWise && !filters.invType ? "border-red-500 focus:border-red-500" : "border-slate-200 dark:border-slate-700 focus:border-blue-500"}`}
            >
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="w-full sm:w-36 shrink-0 space-y-1">
              ...
              <input
                ...
                className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-lg px-3 py-[7px] text-[13px] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer ${submitted && !filters.fromDate ? "border-red-500 focus:border-red-500" : "border-slate-200 dark:border-slate-700"}`}
              />
            </div>
            ...
            <div className="w-full sm:w-36 shrink-0 space-y-1">
              ...
              <input
                ...
                className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-lg px-3 py-[7px] text-[13px] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer ${submitted && !filters.toDate ? "border-red-500 focus:border-red-500" : "border-slate-200 dark:border-slate-700"}`}
              />
            </div>
          </div>
            ...
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                ...
                {visibleColumns.map((col) => (
                  <th
                    key={col}
                    className={`px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500 whitespace-nowrap ${["Amount", "Quantity", "Balance"].includes(col) ? "text-right" : ""}`}
                  >
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr>
                  ...
                  >
                    {submitted
                      ? "No pending values found for selected criteria"
                      : "Click Search to view pending items"}
                  </td>
                </tr>
              ) : (
                data.map((row, idx) => (
                  ...
                  >
                    ...
                    {visibleColumns.map((col) => (
                      <td
                        key={col}
                        className={`px-4 py-3 text-sm ${["Amount", "Quantity", "Balance"].includes(col) ? "text-right font-medium text-slate-700 dark:text-slate-300" : "text-slate-600 dark:text-slate-400"} whitespace-nowrap`}
                      >
                        {col === "Date"
                          ? formatDateStr(row[col])
                          : ["Amount", "Quantity", "Balance"].includes(col)
      ...
      <div className="sticky bottom-0 z-10 bg-brand-yellow dark:bg-brand-yellow/10 border border-brand-yellow/20 dark:border-brand-yellow/5 py-3 px-6 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] rounded-xl flex items-center justify-between">
