...
import { Search, RotateCcw, FileSpreadsheet, FileText, RefreshCw, Loader2, X, FolderOpen, Scale, CheckCircle, Settings, TrendingUp, Info, ShieldCheck, Printer } from 'lucide-react';
...
export const TrialBalanceReport: React.FC = () => {
  ...
  return (
    <div className="font-sans text-slate-700 dark:text-slate-200 pt-4 pb-20">
      {/* Left Column (Header, Filters, Tabs, Table) */}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-5">
          <div className="flex flex-wrap xl:flex-nowrap items-end gap-4">
            ...
            <div className="flex items-center space-x-2 shrink-0 pb-0.5 ml-auto">
              <button onClick={handleSearch} disabled={loading} className="w-10 h-10 rounded-lg bg-[#2D9E75] text-white flex items-center justify-center hover:opacity-90 transition-all shadow-sm disabled:opacity-70">
                {loading ? <Loader2 size={16} className="animate-spin" /> : <Search size={16} />}
              </button>
              <button onClick={handleClear} className="w-10 h-10 rounded-lg bg-lime-500 text-white flex items-center justify-center hover:opacity-90 transition-all shadow-sm">
                <RotateCcw size={16} />
              </button>
              <button disabled className="w-10 h-10 rounded-lg bg-rose-500 text-white flex items-center justify-center hover:opacity-90 transition-all shadow-sm disabled:opacity-70">
                <Printer size={16} />
              </button>
              <button onClick={handleExport} disabled={exportLoading} className="w-10 h-10 rounded-lg bg-emerald-600 text-white flex items-center justify-center hover:opacity-90 transition-all shadow-sm disabled:opacity-70">
                {exportLoading ? <Loader2 size={16} className="animate-spin" /> : <FileSpreadsheet size={16} />}
              </button>
              <button onClick={handleRefreshAll} className="w-10 h-10 rounded-lg bg-blue-500 text-white flex items-center justify-center hover:opacity-90 transition-all shadow-sm">
                <RefreshCw size={16} />
              </button>
            </div>
          </div>
      ...
      {/* Tab Content */}
      <div className="flex flex-col lg:flex-row bg-white dark:bg-slate-900 rounded-b-xl border border-t-0 border-slate-200 dark:border-slate-700 shadow-sm flex-1">
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          {selectedTab === 1 && (
            ...
            {loading ? (
              ...
              <>
                {/* Unified Data Table */}
                <div className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col">
    ...
    {/* End of Left Column */}
      ...
      {selectedTab === 1 && (
        <aside className="w-full xl:w-80 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex flex-col shrink-0 shadow-sm overflow-hidden h-full">
          ...
          <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-8 custom-scrollbar">
            <div className="space-y-4">
              ...
              <div className="relative aspect-square w-full bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center overflow-hidden shadow-inner border border-slate-100 dark:border-slate-700">
              ...
              <div className="space-y-3 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
        ...
        <footer className="sticky bottom-0 z-10 mt-6 bg-brand-yellow dark:bg-brand-yellow/10 px-6 py-4 rounded-xl shadow-[0_-4px_20px_rgba(0,0,0,0.15)] flex flex-wrap items-center justify-between gap-4 border border-brand-yellow/20">
          <div className="flex flex-wrap items-center gap-8 md:gap-12">
            <div className="flex flex-col">
              <span className="text-[10px] uppercase font-bold text-slate-800 dark:text-brand-yellow/90 tracking-widest">Total Debit</span>
              <span className="text-xl font-black text-slate-950 dark:text-brand-yellow tabular-nums">
                {lst.length > 0 ? H.formatNumber(H.getTotalProcessedDebit(lst), precision) : '1,245,600.00'}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] uppercase font-bold text-slate-800 dark:text-brand-yellow/90 tracking-widest">Total Credit</span>
              <span className="text-xl font-black text-slate-950 dark:text-brand-yellow tabular-nums">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-3 px-3 md:px-4 py-2 bg-white/70 dark:bg-emerald-950/20 border border-emerald-500/30 rounded-xl">
              <CheckCircle size={18} className="text-emerald-600 dark:text-emerald-400 hidden md:block" />
              <div>
                <p className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest leading-none mb-0.5">Status</p>
                <p className="text-sm font-bold text-emerald-800 dark:text-emerald-300">Accounts Balanced</p>
              </div>
            </div>
            <div className="flex items-center gap-3 px-3 md:px-4 py-2 bg-white/70 dark:bg-slate-900/10 border border-slate-900/10 dark:border-brand-yellow/20 rounded-xl">
              <div className="text-right">
                <p className="text-[10px] font-bold text-slate-800 dark:text-brand-yellow/90 uppercase tracking-widest leading-none mb-0.5">Difference</p>
                <p className="text-sm font-black text-slate-950 dark:text-brand-yellow">
