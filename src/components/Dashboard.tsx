...
export const Dashboard: React.FC<{
  onModuleClick?: (type: any, title: string) => void;
}> = ({ onModuleClick }) => {
        ...
        <div className="col-span-12 lg:col-span-9 space-y-6">
            ...
            <div className="relative z-10 space-y-2">
              <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                ...
                <span className="text-primary">
                  {user?.first_Name || "Admin"} {user?.lastname || ""}
                </span>
              </h1>
              <p className="text-slate-500 dark:text-slate-400 max-w-lg">
                Manage your business operations seamlessly with the latest
                insights and powerful ERP tools at your fingertips.
              </p>
              <div className="pt-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() =>
                  window.dispatchEvent(new CustomEvent("open-widget-library"))
                }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <ModuleCard
                ...
                color="amber"
                onClick={() =>
                  onModuleClick?.("purchase-invoice-list", "Purchase Invoice")
                }
              />
              <ModuleCard
                icon={<Wallet size={24} />}
                ...
                color="emerald"
                onClick={() =>
                  onModuleClick?.("receipt-voucher-list", "Receipt Vouchers")
                }
              />
              <ModuleCard
                ...
                color="red"
                onClick={() =>
                  onModuleClick?.("payment-voucher-list", "Payment Vouchers")
                }
              />
              <ModuleCard
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <ModuleCard
                ...
                color="emerald"
                onClick={() =>
                  onModuleClick?.("profit-loss", "Profit & Loss Statement")
                }
              />
              <ModuleCard
                ...
                color="indigo"
                onClick={() =>
                  onModuleClick?.("bi-dashboard", "Business Intelligence")
                }
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="h-48 flex items-end gap-2 px-2">
                {isLoading ? (
                  ...
                  purchaseTrend.slice(0, 7).map((item, i) => {
                    const maxAmount =
                      Math.max(...purchaseTrend.map((t) => t.amount || 0)) || 1;
                  ...
                  [40, 65, 45, 80, 95, 50, 75].map((height, i) => (
                    <div
                      ...
                      className={`flex-1 rounded-t-lg transition-all duration-500 ${
                        i === 4
                          ? "bg-primary"
                          : "bg-slate-100 dark:bg-slate-800 hover:bg-primary/40"
                      }`}
                      style={{ height: `${height}%` }}
                    ></div>
                  ))
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              <div className="space-y-4">
                <TaskItem
                  color="bg-red-500"
                  title="Approve Pending Vouchers"
                  subtitle="Finance Team • 2h ago"
                />
                <TaskItem
                  color="bg-amber-500"
                  title="Inventory Restock Alert"
                  subtitle="Warehouse • 5h ago"
                />
                <TaskItem
                  color="bg-blue-500"
                  title="Monthly GST Filing"
                  subtitle="Compliance • 1d ago"
                />
              </div>
            </div>
          </div>
        ...
        <aside className="col-span-12 lg:col-span-3 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              ...
              <span className="w-5 h-5 bg-red-500 text-[10px] text-white flex items-center justify-center rounded-full font-bold">
                4
              </span>
            </div>
...
const ModuleCard = ({
  ...
  const colorClasses: Record<string, string> = {
    ...
    amber:
      "bg-amber-50/50 dark:bg-amber-900/20 border-amber-100 dark:border-amber-800/50 text-amber-500 dark:text-amber-200",
    emerald:
      "bg-emerald-50/50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800/50 text-emerald-500 dark:text-emerald-200",
    red: "bg-red-50/50 dark:bg-red-900/20 border-red-100 dark:border-red-800/50 text-red-500 dark:text-red-200",
    indigo:
      "bg-indigo-50/50 dark:bg-indigo-900/20 border-indigo-100 dark:border-indigo-800/50 text-indigo-500 dark:text-indigo-200",
    slate:
  ...
  return (
    ...
    >
      <div
        className={`w-14 h-14 rounded-2xl bg-white dark:bg-slate-800 shadow-sm mb-4 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors`}
      >
        {icon}
      </div>
...
const NotificationItem = ({
  ...
  return (
    <div className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex gap-3 cursor-pointer">
      <div
        className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${typeClasses[type]}`}
      >
        {icon}
      </div>
      <div>
        <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 leading-tight mb-1">
          {message}
        </p>
